...
export const scrapeCommand = command(
  ...
  async (argv) => {
    ...
    // Validate source
    if (source !== 'leboncoin' && source !== 'autoscout24' && source !== 'malt') {
      if (!json) {
        p.log.error(`Unknown source: ${source}. Supported: leboncoin, autoscout24, malt`);
    ...
    try {
      if (!json) {
        ...
        const strategy = source === 'autoscout24'
          ? new AutoScout24Strategy()
          : source === 'malt'
            ? new MaltStrategy()
            : new LeboncoinStrategy();
